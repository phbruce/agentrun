...
Phase 1 — InfraBot Genesis (Feb 12, 2026)
  Feb 12   InfraBot born in lambda-node-example repo
           First commit: Agent SDK + Slack integration
           Same day: infra moved to aa-iac via Terragrunt
...
Phase 6 — AgentRun Extraction (Mar 1-4, 2026)
  ...
  Mar 2    18 tool manifests in aa-iac pack (.claude/infrabot/tools/*.yaml)
...
Phase 7 — Public Release (Mar 5, 2026)
  Mar 5    phbruce/agentrun repo created (single initial commit)
           ...
           Bedrock Knowledge Base module deployed in aa-iac
...
Phase 8 — Scope Rename & Eval (Mar 7-13, 2026)
  ...
  Mar 12   API Gateway auto-create path segments (sigla-lambda v2.7.0)
...
**Decision**: Revert to Secrets Manager with a shared DB user (`sigla-api`) and increase the upload handler timeout to 30s. The 100 individual users were deleted and Terraform states cleaned via S3.
...
```text
Feb 12 — Inline code in lambda-node-example
